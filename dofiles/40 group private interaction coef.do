...
from a regression restricted to:
    Forward Hindu + comparison group
...
Regression:
    NNM_i = alpha
          + beta_1 ComparisonGroup_i
          + beta_2 Private_i
          + beta_3 ComparisonGroup_i x Private_i
          + X_i gamma
          + district FE
          + error_i

The reported coefficient beta_3 is:

    (Comparison private - Comparison public)
    -
...
foreach region of local regions {
    ...
    *--------------------------------------------------------
    ...
    foreach g of local groups {
        ...
        capture drop comp_group
        gen byte comp_group = .
        replace comp_group = 0 if group == 4
        replace comp_group = 1 if group == `g'

        label define comp_group_lbl ///
            0 "Forward Hindu" ///
            1 "`groupname`g''", replace

        label values comp_group comp_group_lbl


        *----------------------------------------------------
        * Run pairwise interaction regression:
        * comparison group vs Forward Hindu
        * public vs private
        *----------------------------------------------------

        capture noisily reghdfe `outcome' ///
            i.comp_group##i.private ///
            `controls' ///
            [aw = v005] ///
            if `region' == 1 & inlist(group, 4, `g'), ///
        ...
        *----------------------------------------------------
        ...
        quietly count if e(sample) & comp_group == 0 & private == 0
        local n_fwd_public = r(N)

        quietly summarize nd if e(sample) & comp_group == 0 & private == 0, meanonly
        local deaths_fwd_public = r(sum)

        quietly count if e(sample) & comp_group == 0 & private == 1
        local n_fwd_private = r(N)

        quietly summarize nd if e(sample) & comp_group == 0 & private == 1, meanonly
        local deaths_fwd_private = r(sum)

        quietly count if e(sample) & comp_group == 1 & private == 0
        local n_comp_public = r(N)

        quietly summarize nd if e(sample) & comp_group == 1 & private == 0, meanonly
        local deaths_comp_public = r(sum)

        quietly count if e(sample) & comp_group == 1 & private == 1
        local n_comp_private = r(N)

        quietly summarize nd if e(sample) & comp_group == 1 & private == 1, meanonly
        ...
        *----------------------------------------------------
        ...
        if `show_cell' == 1 {
            ...
            local beta = _b[1.comp_group#1.private]

            quietly test 1.comp_group#1.private
            local p = r(p)
